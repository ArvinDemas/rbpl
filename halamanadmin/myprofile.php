...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($is_admin) {
        // Update profile data for admin
        if (isset($_POST['update_profile'])) {
            $nama = mysqli_real_escape_string($konek, $_POST['nama']);
            $email = mysqli_real_escape_string($konek, $_POST['email']);

            $update_query = "UPDATE admin SET nama = ?, email = ? WHERE id_admin = ?";
        ...
        if (isset($_POST['upload_picture']) && isset($_FILES['profile_picture'])) {
            ...
            if (in_array($file['type'], $allowed_types) && $file['error'] === 0) {
                ...
                $query = "SELECT gambar FROM admin WHERE id_admin = ?";
                ...
                if (move_uploaded_file($file['tmp_name'], $destination)) {
                    ...
                    $update_img_query = "UPDATE admin SET gambar = ? WHERE id_admin = ?";
                    $stmt = mysqli_prepare($konek, $update_img_query);
                    mysqli_stmt_bind_param($stmt, "si", $new_filename, $id_admin);
        ...
        if (isset($_POST['delete_picture'])) {
            ...
            // $update_img_query = "UPDATE admin SET gambar = NULL WHERE id_admin = ?";
            // $stmt = mysqli_prepare($konek, $update_img_query);
            // mysqli_stmt_bind_param($stmt, "i", $id_admin);
            // if (mysqli_stmt_execute($stmt)) {
...
// Fetch profile data
if ($is_admin) {
    $query = "SELECT nama, email, gambar FROM admin WHERE id_admin = ?";
    $stmt = mysqli_prepare($konek, $query);
    if (!$stmt) {
    ...
    if (!empty($customer['gambar'])) {
        ...
        $profile_image = '../image/vector.png';
    }
} 
    ...
    <div class="flex flex-col items-center gap-10">
      ...
      <form method="post" action="myprofile.php" class="flex flex-col items-start gap-6" enctype="multipart/form-data">
        ...
        <div class="space-y-3 w-[520px]">
          ...
          <input id="status" name="status" type="text" value="<?php echo $is_admin ? 'Admin' : 'mekanik'; ?>" class="w-full px-4 py-3 border border-white rounded-lg text-white text-sm bg-transparent" readonly />
        </div>
        <?php if ($is_admin): ?>
